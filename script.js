...
function onLoad() {
    ...
    let edgeMat = {
        ...
        vertical: new cv.Mat(),
        horizontalTmp: new cv.Mat(),
        verticalTmp: new cv.Mat(),
    ...
    function processVideo() {
        ...
        try {
            ...
            cap.read(src);
/*
            cv.resize(src, dst, new cv.Size(), 0.5, 0.5, cv.INTER_AREA);

            let ksize = new cv.Size(3, 3);
            cv.GaussianBlur(dst, dst, ksize, 7, 0, cv.BORDER_DEFAULT);
            cv.cvtColor(dst, dst, cv.COLOR_RGBA2GRAY, 0);

            cv.Sobel(dst, dst64F, cv.CV_64F, 0, 1, 3, 1, 0, cv.BORDER_DEFAULT);
    
            cv.convertScaleAbs(dst64F, dst64F, 1, 0);
    
            dst64F.convertTo(dst, cv.CV_8U);
...
*/
            ...
            ctx.fillRect(0, 0, video.width, marginTop);
            ctx.fillRect(0, height + marginTop, video.width, marginTop);
            ctx.fillRect(0, marginTop, marginLeft, height);
            ctx.fillRect(marginLeft + width, marginTop, marginLeft, height);
            ctx.globalAlpha = 1.0;
            ...
            */
            ...
            let topEdges = findPotentialEdges(edgeRect.top, src, edgeMat.horizontal, edgeMat.horizontalTmp, edgeMat.horizontalGray, topScoreFunc);
            let rightEdges = findPotentialEdges(edgeRect.right, src, edgeMat.vertical, edgeMat.verticalTmp, edgeMat.verticalGray, rightScoreFunc);
            let bottomEdges = findPotentialEdges(edgeRect.bottom, src, edgeMat.horizontal, edgeMat.horizontalTmp, edgeMat.horizontalGray, bottomScoreFunc);
            let leftEdges = findPotentialEdges(edgeRect.left, src, edgeMat.vertical, edgeMat.verticalTmp, edgeMat.verticalGray, leftScoreFunc);
...
/*
            ...
            [tl, tr, br, bl].forEach(pt => drawPoint(ctx, pt));
            */
            ...
            //for (let key in edgeRect)
            //   drawRect(ctx, edgeRect[key]);
...
function findPotentialEdges(rect, img, mat, tmpRGBA, matGray, scoreFunc) {
    /*
    let ksize = new cv.Size(5, 5);
    cv.GaussianBlur(img.roi(rect), mat, ksize, 0, 0, cv.BORDER_DEFAULT);
    cv.cvtColor(mat, mat, cv.COLOR_RGBA2GRAY, 0);

    if (rect.width > rect.height)
        cv.Sobel(mat, mat, cv.CV_16S, 0, 1, 5, 1, 0, cv.BORDER_DEFAULT);
    else
        cv.Sobel(mat, mat, cv.CV_16S, 1, 0, 5, 1, 0, cv.BORDER_DEFAULT);

    cv.convertScaleAbs(mat, mat, 1, 0);
...
*/
    ...
    //let ksize = new cv.Size(3, 3);
    //cv.GaussianBlur(mat, mat, ksize, 7, 0, cv.BORDER_DEFAULT);
    //let hsvChannels = new cv.MatVector();

    //cv.cvtColor(mat, mat, cv.COLOR_RGB2HSV);
    //cv.split(mat, hsvChannels);

    
    //cv.GaussianBlur(mat, tmpRGBA, new cv.Size(0, 0), 3);
    //cv.addWeighted(tmpRGBA, 1.5, mat, -0.5, 0, mat);
    ...
    //for (let i = 1; i < 3; i++)
    //    cv.Canny(matGray, matGray, 25, 45, 3, false);
    
    //cv.bitwise_or(channels.get(0), channels.get(1), )

    //hsvChannels.delete();
    /*
    if (rect.width > rect.height)
        cv.Sobel(matGray, mat64F, cv.CV_64F, 0, 1, 3, 1, 0, cv.BORDER_DEFAULT);
    else
        cv.Sobel(matGray, mat64F, cv.CV_64F, 1, 0, 3, 1, 0, cv.BORDER_DEFAULT);

    cv.convertScaleAbs(mat64F, mat64F, 1, 0);

    mat64F.convertTo(matGray, cv.CV_8U);
    */
    //cv.threshold(matGray, matGray, 40, 255, cv.THRESH_BINARY);
